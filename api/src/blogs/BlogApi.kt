...
class getUser @Inject constructor(
    ...
    @Get("/")
    fun getUser(userId: Long):HttpResponse<Any>{
        val blog = blogController.getUser(userId)
        return blog.getHttpResponse()
    }
}
...
@Controller("/api/v1/user/{blogId}")
class fetchBlogById @Inject constructor(
    private val blogController: BlogController
) {
    @Get("/")
    fun fetchBlogById(blogId: Long):HttpResponse<Any>{
        val blog = blogController.fetchBlogById(blogId)
        return blog.getHttpResponse()
    }
}
