...
class BlogRepositoryImpl(
    ...
    override fun createUser(username: String): List<User> {
        ...
        val query =
            dataSource.connection.prepareStatement("INSERT INTO users(name,created_at,updated_at) VALUES(?,?,?)")
        query.setString(1, username)
        query.setTimestamp(2, Timestamp(System.currentTimeMillis()))
        query.setTimestamp(3, Timestamp(System.currentTimeMillis()))
        query.executeUpdate()
        val displayData = dataSource.connection.prepareStatement("SELECT * FROM users ORDER BY id DESC LIMIT 1")
        val result = displayData.executeQuery()
    ...
    override fun createUserBlog(userId: Long, title: String, content: String): List<Blog> {
        ...
        if (result.next()) {
            ...
            val displayInsertData = dataSource.connection.prepareStatement("SELECT * FROM blogs ORDER BY id DESC LIMIT 1")
    ...
    override fun getAll(): List<Blog> {
        ...
        while (result.next()) {
            ...
            val created_at = result.getTimestamp("created_at") as Timestamp
            val updated_at =result.getTimestamp("updated_at") as Timestamp
            val user_id = result.getLong("user_id")
            val published = result.getBoolean("is_publish")
    ...
    override fun updateUserBlog(userId: Long, blogId:Long,title: String, content: String): List<Blog> {
        ...
        if (result.next()) {
            val updateQuery =
                dataSource.connection.prepareStatement("UPDATE blogs SET title=?,content=? updated_at=? where id = ${blogId}")
            ...
            val blog = updateQuery.executeUpdate()
            return updateUserBlog(userId,blogId, title, content)
        }
        return blog
    }
    ...
    override fun deleteUserBlog(userId: Long, blogId: Long): List<Blog> {
        ...
        val result = displayBlog.executeQuery()
        if (result.next()) {
            val updateQuery = dataSource.connection.prepareStatement("DELETE FROM blogs where id = ?")
